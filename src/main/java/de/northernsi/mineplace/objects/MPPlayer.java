...
public class MPPlayer implements Player {
            ...
            public void run() {
                if (!MinePlace.getInstance().usersWithLM.contains(getUniqueId()))
                    sendMessage("§7Hey, we've detected that you're not using LabyMod.\nMight wanna try it? Download" +
                            ...
                            "example: Our team system is only useful when our players are using LabyMod because your" +
                            "team name will be visible right below your ingame name.");
            }
        }.runTaskLater(MinePlace.getInstance(), 20);
        if (getVersion() >= 47 && getVersion() <= 106)
            sendMessage("§e§lMine§6§lPlace §7» §cYour client is way too outdated! Game breaking errors can" +
                    "occur while playing on this server. Please consider upgrading to at least 1.9+!");
        if (!ConfigHandler.getInstance().existsTeam(getTeam()) && getTeam() != null) {
            deleteUserFile();
            sendMessage("§e§lMine§6§lPlace §7» §cYour team got deleted while you were offline!");
        }
